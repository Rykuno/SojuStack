...
3. [Server-Side Performance](#3-server-side-performance) — **HIGH**
   - 3.1 [Authenticate Server Mutations Like API Routes](#31-authenticate-server-mutations-like-api-routes)
   ...
   - 3.8 [Use Background Tasks for Non-Blocking Operations](#38-use-background-tasks-for-non-blocking-operations)
...
In API routes and server-side request handlers, start independent operations immediately, even if you don't await them yet.
...
Use `import()` and `React.lazy()` to lazy-load large components not needed on initial render.
...
### 3.1 Authenticate Server Mutations Like API Routes
...
Frameworks and runtimes may already memoize certain network requests within a single request cycle. Even when they do, `React.cache()` is still useful for other async tasks:
...
### 3.8 Use Background Tasks for Non-Blocking Operations
